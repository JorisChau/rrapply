...
element through its principal argument and there is no convenient way to access the list element its name or location from
...
A primary use of \code{feverywhere = TRUE} is to perform node aggregation by calculating some type of summary statistic over sublists of the nested list. Together with the \code{.xname} and \code{.xpos} arguments, we have flexibility in deciding which sublists to summarize or collapse trough the \code{condition} function.\\[3mm]
...
As \code{rrapply} is written in \textsf{R}'s internal \textsf{C} API, it is computationally significantly more efficient than its equivalent implementation based on recursion through a nested list in interpreted \textsf{R}. In the figures below, several benchmark timings of \code{rrapply} against that of base \code{rapply} are displayed in the context of data.frame manipulation. More precisely, we time the application of a dummy function \code{f} (unit multiplication) respectively to each column and to each \emph{numeric} column of a data.frame \code{x} of size $(M \times N)$, where the entries of the data.frame \code{x} are randomly sampled from a uniform distribution on the unit interval.\\[3mm]
