...
By construction, both base \code{rapply} and \code{rrapply} apply the \code{f} function only to leaf elements of a nested list by recursing further into any list-like element that is encountered. In \code{rrapply}, it is possible to override this default behavior using the argument \code{feverywhere = TRUE}. In this case, \code{rrapply} does not automatically recurse deeper into list-like objects, and the \code{f} function will be applied to any element (e.g. a sublist) that satisfies the \code{condition} function. If the \code{condition} function is not satisfied for a list-like element, \code{rrapply} will recurse further into the sublist, apply the \code{f} function to the nodes that satisfy the \code{condition}, and so on.\\[3mm]
A primary use of \code{feverywhere = TRUE} is to perform node aggregation by calculating some type of summary statistic over sublists of the nested list. Together with the \code{.xname} and \code{.xpos} symbols, we have flexibility in deciding which sublists to summarize or collapse trough the \code{condition} function.\\[3mm]
...
@
\begin{remark}
Note that the \code{x} argument in the \code{f} function is now the entire sublist for which the \code{condition} is satisfied. For this reason, we first need to \code{unlist} the sublist in order to supply a numeric vector to \code{mean} and \code{sd}.
\end{remark}
...
individual columns of a data.frame. Sometimes we wish to apply \code{f} to a data.fame object as a whole, instead of its individual
columns, which is not possible with \code{rapply}. For this purpose, \code{rrapply} includes a convenience argument \code{dfaslist}. If \code{dfaslist = TRUE}, \code{rrapply} behaves in
...
\begin{remark}
Note that the same result can also be obtained using \code{feverywhere = TRUE} and checking that the list element under evaluation is a data.frame:
<<echo=FALSE>>=
rrapply(
  oceania_df,
...
@
Using the \code{dfaslist} argument is somewhat more concise and also slighty more efficient as the class of the list element is checked directly analogous to the \code{classes} argument. 
\end{remark}
...
As \code{rrapply} is written in \textsf{R}'s internal \textsf{C} API, it is computationally significantly more efficient than its equivalent implementation based on recursion through a nested list in interpreted \textsf{R}. In the figures below, several benchmark timings of \code{rrapply} against that of base \code{rapply} are displayed in the context of data.frame manipulation. In particular, we time the application of a dummy function \code{f} (unit multiplication) respectively to each column and to each \emph{numeric} column of a data.frame \code{x} of size $(M \times N)$, where the entries of the data.frame \code{x} are randomly sampled from a uniform distribution on the unit interval.\\[3mm]
The displayed timings are the median computation times (ms) of 100 evaluations of the benchmarked expressions in R 3.6.2 on a single-core processor (Intel i7-8550U, 1.80 GHz, 16 GiB system memory). In the left-hand plots, the number of data.frame rows is fixed at $M = 1000$ and the number of columns increases from $N = 100$ to $N = 10\ 000$, and in the right-hand plots the number of data.frame columns is fixed at $N = 100$ and the number of rows increases from $M = 1000$ to $M = 10^6$.
\\[3mm]
...
\begin{remark}
The same processing tasks executed with \texttt{data.table} (v1.8.2) and \texttt{dplyr} (v0.8.4) resulted in roughly equivalent computation times. These results have not been included in the figures as it is difficult to make an objective comparison between R-packages. For instance, \texttt{data.table} at this moment does not include a native procedure to perform conditional column mutations, whereas \texttt{dplyr} does with \code{mutate\_if}.
\end{remark}
