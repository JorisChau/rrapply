...
max(sublist_count)
@
The \code{.xsiblings} argument allows us to evaluate the sibling nodes of the current list element under evalution. An example usage is to look up all direct neighbors of a particular country in the nested list:
<<>>=
